...
def fetch_pypi(recipe_info):
    name = recipe_info["pypi"]["name"]
    version_pattern = re.compile(recipe_info["pypi"]["version_pattern"])
    ...
    for version, release_packages in project["releases"].items():
        ...
        for package in release_packages:
            ...
            newest = {
                "pv": pv,
                "commit_date": package["upload_time"],
                "md5sum": package["digests"]["md5"],
                "sha256sum": package["digests"]["sha256"],
            }
            packages.append(newest)

    if recipe_info["pypi"]["version_order"] == "semver":
        newest = max(packages, key=semver_key)

    recipe_info.update(newest)


def fetch_sourceforge(recipe_info):
    project = recipe_info["sourceforge"]["project"]
    version_pattern = re.compile(recipe_info["sourceforge"]["version_pattern"])

    best_release = get_json(
        f"https://sourceforge.net/projects/{project}/best_release.json"
    )

    recipe_info["pv"] = version_pattern.match(best_release["release"]["filename"])[1]
    recipe_info["commit_date"] = best_release["release"]["date"]
...
def fetch_info(recipe_info):
    ...
    elif "pypi" in recipe_info:
        fetch_pypi(recipe_info)

    elif "sourceforge" in recipe_info:
        fetch_sourceforge(recipe_info)
